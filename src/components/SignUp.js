...
export default function SignUp({navigation}) {
  ...
  const handleSubmit = () => {
    ...
    if (!fullNameRegex.test(state.name)) {
      ...
      alert(
        'Invalid mobile number. Please enter a valid mobile number ex: +919876543210.',
      );
    } else if (!emailRegex.test(state.email)) {
      alert(
        'Invalid Email, Please enter a valid email address ex: user@email.com',
      );
    } else if (!dobRegex.test(state.dob)) {
      alert(
        'Invalid Date of Birth. Please enter a valid Date of Birth ex: DD-MM-YYYY',
      );
    } else if (!isValidGender(state.Gender)) {
      alert(
        'Invalid Gender. Please enter a valid gender male, female and other',
      );
    } else if (!isValidType(state.type)) {
      alert('Invalid type. Please enter a valid type Business or Personal');
    } else {
      alert(
        `${state.name}, ${state.mobileNumber}, ${state.email}, ${state.dob}, ${state.Gender}, ${state.type}`,
      );
      ...
      <KeyboardAvoidingView style={styles.container}>
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={styles.SubmitBtn}>
            <Text
              style={{fontSize: 20, fontWeight: '300', color: '#fff'}}
          ...
          <TouchableOpacity style={styles.resetCancel} onPress={handleReset}>
            <Text style={{fontSize: 18, fontWeight: '300', color: '#000000'}}>
              {' '}
              Reset{' '}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.resetCancel}>
            <Text
              style={{fontSize: 18, fontWeight: '300', color: '#000000'}}
